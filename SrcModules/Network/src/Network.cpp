...
extern "C" {
__declspec(dllexport) zia::api::Net *create() { return new (nzm::Network); }
}

#elif defined(__GNUC__) || defined(__GNUG__)

extern "C" {
zia::api::Net *create() { return new (nzm::Network); }
}
...
bool nzm::Network::config(const zia::api::Conf &conf) {
  try {
    _port = std::get<long long>(conf.at("port").v);
  } catch (...) {
    nz::Log::warning("port not found or must be a number, the default port has "
                     "been set to '7000'",
                     "Network Module");
    _port = 7000;
  }
...
bool nzm::Network::run(zia::api::Net::Callback cb) {
  ...
  auto funcRunSelect = std::bind(&nzm::Network::runSelect, this,
...
bool nzm::Network::send(zia::api::ImplSocket *sock,
