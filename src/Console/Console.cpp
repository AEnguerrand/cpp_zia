...
nz::Console::Console(nz::zia &zia) : _zia(zia) {
  commands_sg = {
      {1,
      ...
      {3,
       {{{"modules", "add", "<module name>"},
         [&](auto &args) { _zia.getModulesLoader().addModule(args.at(2)); }},
        {{"modules", "remove", "<module name>"},
         [&](auto &args) {
           _zia.getModulesLoader().deleteModuleByName(args.at(2));
         }},
        {{"network", "set", "<module name>"},
         [&](auto &args) { _zia.setModuleNetwork(args.at(2)); }}}},
  };
}
...
void nz::Console::dispatchCommand(const std::vector<std::string> &args) {
  ...
  for (auto &commandInfos : commands) {
    ...
    for (uint32_t i = 0; i < size; ++i) {
      if ((cmd.at(i).front() != '<' && cmd.at(i).back() != '>') &&
          args.at(i) != cmd.at(i))
        same = false;
    }
    if (same) {
...
void nz::Console::invalidCommand() { Log::print("Invalid command"); }
