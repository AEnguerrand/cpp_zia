...
zia::api::http::Method
nz::HttpParser::GetMethodFromString(const std::string &input) {
...
zia::api::http::Version
nz::HttpParser::GetVersionFromString(const std::string &input) {
...
std::string
nz::HttpParser::GetStringFromVersion(const zia::api::http::Version &input) {
  static const std::map<std::string, zia::api::http::Version> link = {
      {"HTTP/0.9", zia::api::http::Version::http_0_9},
      {"HTTP/1.0", zia::api::http::Version::http_1_0},
      {"HTTP/1.1", zia::api::http::Version::http_1_1},
...
zia::api::HttpRequest
nz::HttpParser::GetRequest(const std::vector<std::string> &input) {
  size_t pos = 0;
...
zia::api::HttpResponse
nz::HttpParser::GetResponse(const zia::api::Net::Raw &input) {
  return GetResponse(transform::RawToString(input));
}

zia::api::HttpResponse nz::HttpParser::GetResponse(const std::string &input) {
  std::vector<std::string> row =
      transform::Split(transform::EpurStr(input, CRLF), CRLF);
  return GetResponse(row);
}

zia::api::Net::Raw
nz::HttpParser::ResponseToRaw(const zia ::api::HttpResponse &input,
...
zia::api::HttpResponse
nz::HttpParser::GetResponse(const std::vector<std::string> &input) {
  size_t pos = 0;
  ...
  // Get the first line ==> contain the http version, the status code and the
  // reason
  if (data.size() > 0) {
    std::string fLine = data.at(0);
    data.erase(data.begin());
...
zia::api::HttpDuplex nz::HttpParser::Parse(const zia::api::Net::Raw &raw) {
  ...
  std::vector<std::string> row =
      transform::Split(transform::EpurStr(request, CRLF), CRLF);
...
void nz::HttpParser::CheckRequestValidity(zia::api::HttpRequest data) {
  ...
  if (data.method == zia::api::http::Method::options) {
    if (!data.body.empty() ||
        data.headers.find("Content-Length") != data.headers.end() ||
        data.headers.find("Transfer-Encoding") != data.headers.end()) {
      if (data.headers.find("Content-Type") != data.headers.end())
        throw HttpParserException("This method require a 'Content-type' in "
                                  "header because it contains a body");
    }
  }
...
void nz::HttpParser::CheckResponseValidity(
    zia::api::HttpResponse data, const zia::api::http::Method &method) {
  // Check Status
