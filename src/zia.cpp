...
nz::zia::zia()
    : _isStart(false), _process(_modulesLoader), _parser(_process, nullptr),
...
void nz::zia::start() {
  if (_isStart) {
    Log::error("The server's already started, please consider stopping it "
               "before trying again",
               "Zia Core", 2);
    return;
  }
...
void nz::zia::loadConf() {
  ...
  try {
    _moduleNet = std::get<std::string>(_conf.at("module_net").v);
  } catch (...) {
    _moduleNet = "cpp_zia_module_network";
    nz::Log::warning("module_net not found or must be a string, the default "
                     "network module has been set to '" +
                         _moduleNet + "'",
                     "Zia Core");
  }
  ...
  try {
    ::zia::api::ConfArray modules =
        std::get<::zia::api::ConfArray>(_conf.at("modules").v);
    for (auto module : modules) {
      _modules.push_back(std::get<std::string>(module.v));
    }
  } catch (...) {
    _modules.push_back("cpp_zia_module_router");
    nz::Log::warning("modules not found or must be an array, the default "
                     "module has been set to '" +
                         _modules.back() + "'",
                     "Zia Core");
  }
  ...
  try {
    ::zia::api::ConfArray modulesPath =
        std::get<::zia::api::ConfArray>(_conf.at("modules_path").v);
    for (auto modulePath : modulesPath) {
      _modulesPath.push_back(std::get<std::string>(modulePath.v));
    }
  } catch (...) {
    _modulesPath.push_back("./Modules");
    nz::Log::warning("modules_path not found or must be an array, the default "
                     "module path has been set to '" +
                         _modulesPath.back() + "'",
                     "Zia Core");
  }
  ...
  // Get module_net (string) from config file
  try {
    _debug = std::get<bool>(_conf.at("debug").v);
  } catch (...) {
    _debug = false;
    nz::Log::warning(
        "debug not found or must be a boolean, debug mode set to 'false'",
        "Zia Core");
  }

  // Get module_net (string) from config file
  try {
    _logLevel = std::get<long long>(_conf.at("log_level").v);
  } catch (...) {
    _logLevel = 1;
    nz::Log::warning(
        "log_level not found or must be a number, default log level set to '" +
            std::to_string(_logLevel) + "'",
        "Zia Core");
  }
}
...
void nz::zia::loadNetwork() {
  ...
  for (auto &path : _modulesPath) {
    for (auto &p : std::filesystem::directory_iterator(path)) {
      if (std::filesystem::is_regular_file(p) &&
        ...
        _moduleNetPath = std::filesystem::path(p).string();
  ...
  ::zia::api::Net::Callback funcCallback =
      std::bind(&nz::Parser::callbackRequestReceived, _parser,
                std::placeholders::_1, std::placeholders::_2);
  _net->run(funcCallback);
  Log::inform("Network is run");
}

nz::ModuleLoader &nz::zia::getModulesLoader() { return _modulesLoader; }
